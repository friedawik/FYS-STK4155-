...
# Make dataseta
X_train, X_test, z_train, z_test = train_test_split(X_scaled, y_scaled, test_size=0.2) #, random_state=42)


lmbd=0.01
GD = GradientDescent(X_train, X_test, z_train, z_test, eta=0.0001, epochs=10 ,batch_size=10, lmbd=lmbd)
beta_sgd = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='plain')
beta_gd = GD.gradient_descent(method='Ridge', learning_schedule='plain')
beta_sgd_jax = GD.stochastic_gradient_descent_jax(method='Ridge', learning_schedule='plain')
beta_gd_jax = GD.gradient_descent_jax(method='Ridge', learning_schedule='plain')
...
plt.grid()
plt.legend()
plt.title('Plain Ridge with JAX gradient')
plt.savefig('ridge_jax_solution_sgd_vs_gd.png')
...
plt.savefig('ridge_jax_vs_analytical.png')
...
for i, eta in enumerate(eta_values): 
    t = time.time()
    GD = GradientDescent(X_train, X_test, z_train, z_test, eta=eta, epochs=10 ,batch_size=batch_size, lmbd=lmbd) 
    scores['Eta'][i] = eta  
    ...
    scores['Eta'][i] = eta  
    scores['Number of epochs'][i]=epochs
    scores['Beta'][i] = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='momentum')
    scores['MSE'][i] = GD.MSE()
    scores['R2'][i] = GD.R2()
    ...
    scores['Eta'][i] = eta  
    scores['Number of epochs'][i]=epochs
    scores['Beta'][i] = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='adagrad')
    scores['MSE'][i] = GD.MSE()
    scores['R2'][i] = GD.R2()
    ...
    scores['Eta'][i] = eta  
    scores['Number of epochs'][i]=epochs
    scores['Beta'][i] = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='adam')
    scores['MSE'][i] = GD.MSE()
    scores['R2'][i] = GD.R2()
    ...
    scores['Eta'][i] = eta  
    scores['Number of epochs'][i]=epochs
    scores['Beta'][i] = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='rms_prop')
    scores['MSE'][i] = GD.MSE()
    scores['R2'][i] = GD.R2()
...
# beta = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='plain')
# r2=GD.R2()
...
# Test different lambda and eta
sns.set()
eta_vals=np.logspace(-8, -3, 6)
lmbd_vals=np.logspace(-8, 1, 6)
mse = np.zeros((len(eta_vals), len(lmbd_vals)))


for i, eta in enumerate(eta_vals):
    for j, lmbd in enumerate(lmbd_vals):
        GD = GradientDescent(X_train, X_test, z_train, z_test, eta=eta, epochs=10 ,lmbd=lmbd, batch_size=10)
        beta = GD.stochastic_gradient_descent(method='Ridge', learning_schedule='plain') 
        z_pred = X_test @ beta
        mse[i][j] = MSE(z_test, z_pred)
        print('mse: ' + str(mse[i][j]))
